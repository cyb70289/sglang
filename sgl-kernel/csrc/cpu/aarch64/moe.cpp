...
void fused_experts_int8_kernel_impl(
    ...
    const std::map<int, std::vector<std::tuple<int, float>>>& x_per_expert,
    int64_t M,
    int64_t N,
    int64_t K,
    int64_t E,
...
void fused_experts_int8_kernel_impl<at::BFloat16>(
    at::BFloat16* __restrict__ y,         // [M, K]
    const int8_t* __restrict__ x,         // [M, K]
    const int8_t* __restrict__ w13,       // [E, 2N, K]
    const int8_t* __restrict__ w2,        // [E, K, N]
    const float* __restrict__ x_scale,    // [M]
    const float* __restrict__ w13_scale,  // [E, 2N]
    const float* __restrict__ w2_scale,   // [E, K]
    const std::map<int, std::vector<std::tuple<int, float>>>& x_per_expert,
    int64_t M,
    int64_t N,
    int64_t K,
    int64_t E,
    int64_t topk) {
  // x dispatch buffer to aggregate all rows to one expert
  ...
  for (const auto& [eid, rows] : x_per_expert) {
    ...
    // up *= silu(gate)
    {
      ...
      const int slice_size = (n_agg * K * sizeof(int8_t)) > (1 << 20) ? 64 : 8;
    ...
    // quantize
    {
      const int64_t grain = 65536LL / (K * sizeof(float));
    ...
    // down = up @ w2
    {
      ...
      const int slice_size = (n_agg * N * sizeof(int8_t)) > (1 << 20) ? 64 : 8;
  ...
  {
    bfloat16_t* y_ptr = reinterpret_cast<bfloat16_t*>(y);
    for (int64_t i = 0; i < M * K; ++i) {
      y_ptr[i] = vcvth_bf16_f32(out[i]);
    }
  }
}
...
at::Tensor fused_experts_cpu(
  ...
  // expert: related input rows and weights
  std::map<int, std::vector<std::tuple<int, float>>> x_per_expert;
  {
  ...
  AT_DISPATCH_REDUCED_FLOATING_TYPES(st, "fused_experts_kernel_impl", [&] {
    if (use_int8_w8a8) {
      ...
      const int64_t grain = 65536LL / (K * sizeof(scalar_t));
