...
__attribute__((target("+dotprod+bf16"))) void sdot_matmul(
  ...
  for (; row + R <= M; row += R) {
    ...
    for (; col + C <= slice_width; col += C) {
      ...
      // TODO: accumulated integer sum may overflow when K >= (1 << 16)
      int k = 0;
      for (; k + 16 <= K; k += 16) {
        int8x16_t va[R];
        int8x16_t vb[C];
...
// TODO: optimize for BF16, Float32
template <typename scalar_t>
void quantize_row_int8(
    int8_t* __restrict__ Aq, float* As, const scalar_t* __restrict__ A, int64_t K, float eps = 1e-7) {
  ...
  for (int64_t k = 0; k < K; ++k) {
    ...
    Aq[k] = static_cast<int8_t>(std::round(val));
  }
  *As = scale;
}
